// Old version.  Due to receiving 400 error from Amadeus, will try to connect to it using its NodeJS SDK instead
...
const makeFlightPriceRequestWithToken = async (token, flight, limit) => {
  await limit(async () => {
    ...
    console.log(
      `${flight.cloudy_orig_city}-${flight.sunny_dest_city} --- request body obj: ${body}`
    );
